from util3 import make_env, plot_learning_curve
...
if __name__ == '__main__':
    ...
    headless = False
    env = make_env("smarts.env:hiway-v0", agent_specs, 'scenarios/sumo/3agents_nohd', headless)
    
    agent_007 = DuelingDQNAgent(gamma=0.99, epsilon=1.0, lr=0.0001, input_dims=(env.observation_space.shape), n_actions=4, mem_size=20000, eps_min=0.01, batch_size=32, replace=1000, eps_dec=1e-5, chkpt_dir='results/dqn', algo='DuelingDQNAgent', env_name='3agents_nohd')

    agent_008 = DuelingDQNAgent(gamma=0.99, epsilon=1.0, lr=0.0001, input_dims=(env.observation_space.shape), n_actions=4, mem_size=20000, eps_min=0.01, batch_size=32, replace=1000, eps_dec=1e-5, chkpt_dir='results/dqn', algo='DuelingDQNAgent', env_name='3agents_nohd')
    
    agent_009 = DuelingDQNAgent(gamma=0.99, epsilon=1.0, lr=0.0001, input_dims=(env.observation_space.shape), n_actions=4, mem_size=20000, eps_min=0.01, batch_size=32, replace=1000, eps_dec=1e-5, chkpt_dir='results/dqn', algo='DuelingDQNAgent', env_name='3agents_nohd')
    ...
    for i in range(n_games):
        ...
        while (len(observations.keys()) < 3) or (agent_007_starting_pos != agent_007_actual_pos) or (agent_008_starting_pos != agent_008_actual_pos):
            observations = env.reset()
            _, _, _, infos = env.step({'Agent-007': 'slow_down', 'Agent-008': 'slow_down', 'Agent-009': 'slow_down'})
            agent_007_actual_pos = infos['Agent-007']['env_obs'][5].road_id
            agent_008_actual_pos = infos['Agent-008']['env_obs'][5].road_id
        ...
        while not any(dones):
            ...
            agent_actions = {'Agent-007': action_dict[action_007], 'Agent-008': action_dict[action_008], 'Agent-009': action_dict[action_009]}
            observations_, rewards, dones, infos = env.step(agent_actions)
            score += sum(rewards)
            dones = [dones['Agent-007'], dones['Agent-008'], dones['Agent-009']]
