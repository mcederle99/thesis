...
from qmix.qmix_smac import QMIX_SMAC
from qmix.normalization import Normalization

from util2_qmix import make_env
from smarts.core.agent_interface import AgentInterface, AgentType
from smarts.core.agent import Agent
from smarts.zoo.agent_spec import AgentSpec
from smarts.core.controllers import ActionSpaceType

map_actions = {0: 'keep_lane', 1: 'slow_down', 2: 'change_lane_left', 3: 'change_lane_right'}
...
class Runner_QMIX_SMAC:
    def __init__(self, args, env_name, number, seed):
        ...
        # Create env
        agent_spec = AgentSpec(interface=AgentInterface.from_type(AgentType.Laner, max_episode_steps=None))
        agent_specs = {'Agent-007': agent_spec, 'Agent-008': agent_spec}
        headless = True 
        self.env = make_env('smarts.env:hiway-v0', agent_specs, 'scenarios/sumo/2agents_nohd', headless)
        self.env_info = {'n_agents': 2, 'obs_shape': 10, 'state_shape': 20, 'n_actions': 4, 'episode_limit': 75} 
        ...
        self.writer = SummaryWriter(log_dir='./results/qmix/runs/{}/{}_env_{}_number_{}_seed_{}'.format(self.args.algorithm, self.args.algorithm, self.env_name, self.number, self.seed))
    ...
    def evaluate_policy(self, ):
        ...
        # Save the win rates
        np.save('./results/qmix/data_train/{}_env_{}_number_{}_seed_{}.npy'.format(self.args.algorithm, self.env_name, self.number, self.seed), np.array(self.win_rates))
    ...
    def run_episode_smac(self, evaluate=False):
        ...
        obs = self.env.reset()
        obs, _, _, infos = self.env.step({'Agent-007': 'slow_down', 'Agent-008': 'slow_down'})
        ...
        for episode_step in range(self.args.episode_limit):
            ...
            obs_n = np.array(list(obs.values())).reshape((2,10))
            assert obs_n.shape == (2,10)
            #s = self.env.get_state()  # s.shape=(state_dim,)
            s = obs_n.reshape(self.args.state_dim)
            #avail_a_n = self.env.get_avail_actions()  # Get available actions of N agents, avail_a_n.shape=(N,action_dim)
            avail_a_n = np.array([[0, 1, 2, 3], [0, 1, 2, 3]])
            assert avail_a_n.shape == (2,4)
            epsilon = 0 if evaluate else self.epsilon
            a_n = self.agent_n.choose_action(obs_n, last_onehot_a_n, avail_a_n, epsilon)
            discrete_actions = [map_actions[a_n[0]], map_actions[a_n[1]]]
            last_onehot_a_n = np.eye(self.args.action_dim)[a_n]  # Convert actions to one-hot vectors
            obs, r, done, info = self.env.step({'Agent-007': discrete_actions[0], 'Agent-008': discrete_actions[1]})  # Take a step
            ...
            if not evaluate:
                ...
                """
                if any(list(done.values())) and episode_step + 1 != self.args.episode_limit:
                    dw = True
                else:
                    dw = False
