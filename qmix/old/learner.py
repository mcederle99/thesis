...
from qmix.replay_memory import MemoryBuffer
from qmix.networks import RNNAgent, QMixer
...
class QMIX():
    ...
    def choose_actions(self, old_actions, observations):
        ...
        for i in range(self.n_agents):
            ...
            agent_obs = T.cat((agent_obs, old_acts)).reshape(-1, 9)
    ...
    def sample_memory(self):
        ...
        for i in range(self.n_agents):
            ...
            obs[i] = T.tensor(obs[i], dtype=T.float).reshape(8,50*self.batch_size,1)
            actions[i] = T.tensor([actions[i]], dtype=T.float).reshape(1,50*self.batch_size,1)
            rewards[i] = T.tensor(rewards[i], dtype=T.float)
            obs_[i] = T.tensor(obs_[i], dtype=T.float).reshape(8,50*self.batch_size,1)
    ...
    def learn(self):
        ...
        for i in range(self.n_agents):
            ...
            agent_q_pred, hidden_states = self.agents_nets[i].forward(T.cat((obs[i][:,0], old_actions[i][:,0])).reshape(1,9), hidden_states)
            agent_q_next, target_hidden_states = self.target_agents_nets[i].forward(T.cat((obs_[i][:,0], actions[i][:,0])).reshape(1,9), target_hidden_states) 
            for k in range(1, self.batch_size*50):
                tmp, hidden_states = self.agents_nets[i].forward(T.cat((obs[i][:,k], old_actions[i][:,k])).reshape(1,9), hidden_states)
                agent_q_pred = T.cat((agent_q_pred, tmp))
                tmp1, target_hidden_states = self.target_agents_nets[i].forward(T.cat((obs_[i][:,k], actions[i][:,k])).reshape(1,9), target_hidden_states)
