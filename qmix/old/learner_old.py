...
class QMIX():
    ...
    def learn(self):
        ...
        for i in range(self.n_agents):
            hidden_states = self.agents_nets[i].init_hidden(first_dim=50*self.batch_size)
            target_hidden_states = self.target_agents_nets[i].init_hidden(first_dim=50*self.batch_size)
            agent_q_pred, _ = self.agents_nets[i].forward(T.cat((obs[i], old_actions[i])).reshape(50*self.batch_size,9), hidden_states)
            pippo = np.zeros(len(indices), dtype=np.int64)
            for j in range(len(indices)):
                pippo[j] =  actions[i][0][j].item()
            agent_q_pred = agent_q_pred[indices, pippo]
            agent_q_next, _ = self.target_agents_nets[i].forward(T.cat((obs_[i], actions[i])).reshape(50*self.batch_size,9), target_hidden_states)
