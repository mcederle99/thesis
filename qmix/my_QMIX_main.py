...
from util_qmix import make_env
...
class Runner_QMIX_SMAC:
    def __init__(self, args, env_name, number, seed):
        ...
        # Create env
        agent_spec = AgentSpec(interface=AgentInterface.from_type(AgentType.Laner, max_episode_step=50))
        ...
        self.env_info = {'n_agents': 2, 'obs_shape': (8,), 'state_shape': (16,), 'n_actions': 4, 'episode_limit': 50} 
    ...
    def run_episode_smac(self, evaluate=False):
        win_tag = False
        episode_reward = 0
        agent_007_starting_pos = 'edge-west_WE'
        obs = self.env.reset()
        ...
        for episode_step in range(self.args.episode_limit):
            ...
            obs_n = list(obs.values())
            assert obs_n.shape == (2,8)
            ...
            obs, r, done, info = self.env.step({'Agent-007': a_n[0], 'Agent-008': a_n[1]})  # Take a step
